...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for ghost, scaredTime in zip(newGhostStates, newScaredTimes):
            ...
            ghost_distance = distance(newPos, ghost_pos)
            min_distance_to_ghost = min(min_distance_to_ghost, ghost_distance)
            
        if scaredTime > 0:
            scared_ghost_bonus += 10 / (ghost_distance + 1)
        
        for i in range(newFood.width):
            for j in range(newFood.height):
                if newFood[i][j]:
                    food_distance = distance(newPos, (i, j))
                    min_distance_to_food = min(min_distance_to_food, food_distance)
                    food_bonus += 10 / (food_distance + 1)

        if action == 'Stop':
            return -float('inf')

        if min_distance_to_ghost <= 1:
            return -1000 + food_bonus + scared_ghost_bonus

        if successorGameState.getNumFood() < currentGameState.getNumFood():
            return 1000 + food_bonus + scared_ghost_bonus - 2 * min_distance_to_ghost

    # Prioritize eating scared ghosts or collecting food
        return food_bonus + scared_ghost_bonus - 2 * min_distance_to_food + 100 / (min_distance_to_ghost + 1)
...
def betterEvaluationFunction(currentGameState):
    ...
    closestFoodDistance = min(util.manhattanDistance(food, pacmanPosition) for food in foodLocations)
    
    # Subtract the closest food distance from the current game state score
    return gameStateScore - closestFoodDistance
